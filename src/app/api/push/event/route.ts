...
export async function POST(req: Request) {
  ...
  const { data: notificationUuid, error } = await supabaseServerAdmin.rpc(
    "notify_user",
    {
      ...
      p_title: title,
      p_body: bodyText,
      p_url: typeof body.url === "string" && body.url.trim()
        ? body.url.trim()
        : "/",
      p_scheduled_for: typeof body.scheduledFor === "string" &&
  ...
  return NextResponse.json({ notificationId: notificationUuid ?? null }, {
    status: 200,
  });
}
