...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn(
        "Error fetching balance data for consolidated response:",
        balanceError,
      );
    ...
    try {
      ...
      console.warn(
        "Error fetching themes data for consolidated response:",
        themesError,
      );
  ...
  try {
    ...
    if (action === "recalculate_points") {
      try {
        ...
        const totalPoints =
          userAchievementsWithPoints?.reduce(
            (sum, ua: any) => sum + (ua.achievements?.points_reward || 0),
            0,
          ) || 0;
    ...
    if (action === "test_coins") {
      ...
      if (coinError) {
        ...
        return NextResponse.json(
          { error: "Failed to award test coins" },
          { status: 500 },
        );
      }
    ...
    if (action === "direct_unlock") {
      ...
      if (!achievementId) {
        return NextResponse.json(
          { error: "Achievement ID is required for direct unlock" },
          { status: 400 },
        );
      }
    ...
    if (!userStats) {
      ...
      const { data: newUserStats, error: createError } =
    ...
    if (action === "task_completed") {
      ...
      const { data: updatedUserStats, error: statsUpdateError } =
        await supabaseServerAdmin
          .from("user_stats")
      ...
      if (statsUpdateError) {
        console.error(
          "Error updating tasks_completed count:",
          statsUpdateError,
        );
    ...
    if (action === "stress_relief_session_started") {
      ...
      if (statsUpdateError) {
        console.error(
          "Error updating stress_relief_sessions_count:",
          statsUpdateError,
        );
      } else {
        userStats = updatedUserStats;
    ...
    if (
      ...
      if (statsUpdateError) {
        console.error(
          "Error updating crashout_reactions_count:",
          statsUpdateError,
        );
      } else {
        userStats = updatedUserStats;
    ...
    for (const achievement of relevantAchievements) {
      ...
      const criteriaCheck = checkAchievementCriteria(
        achievement,
        action,
        value,
        userStats,
      ...
      if (criteriaCheck) {
        ...
        if (!unlockError) {
          ...
          const newTotalPoints =
            (userStats?.total_points || 0) + achievement.points_reward;
          ...
          if (statsUpdateError) {
            ...
            await checkAndUnlockPointsAchievements(
              userId,
...
function checkAchievementCriteria(
  ...
  switch (achievement.type) {
    ...
    case "tutorial":
      ...
      if (
        criteria.action === "onboarding_complete" &&
    ...
    case "wellness":
      ...
      if (
        ...
        action === "wellness_action" &&
        metadata?.wellnessAction === "first_crashout_post"
      ) {
        return true;
      }
      ...
      if (
        criteria.action === "stress_relief_sessions" &&
        userStats?.stress_relief_sessions_count
      ) {
    ...
    case "social":
      if (
        criteria.action &&
        action === "social_action" &&
        metadata?.socialAction === criteria.action
      ) {
        return true;
      }
      if (
        criteria.action === "crashout_reactions" &&
        userStats?.crashout_reactions_count
      ) {
...
async function checkAndUnlockPointsAchievements(
  userId: string,
  totalPoints: number,
  client: any,
) {
