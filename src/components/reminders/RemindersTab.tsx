...
const RemindersTab = () => {
  ...
  const { user } = useUser();
  const { reminders, loading, error, editReminder, addReminder } =
    useReminders();
  ...
  const userStats = {
    user_id: user?.id || "",
    total_points: achievements.reduce(
      (sum, a) => sum + (a.unlocked ? a.points_reward : 0),
      0,
    ),
    level: 1,
    current_streak: 0,
  ...
  const [stuAnimation, setStuAnimation] = useState<
  ...
  >("active");
  const [showCompletedFromLast7Days, setShowCompletedFromLast7Days] =
    useState(true);
  ...
  const showStuMessage = (
    message: string,
    animation: "talking" | "excited" = "talking",
    duration = 3000,
  ) => {
  ...
  useEffect(() => {
    if (!loading) {
      const upcoming = reminders.find(
        (r) => !r.completed && isToday(new Date(r.due_date)),
      );
      if (upcoming) {
        showStuMessage(
          stuMessages.reminderDue.replace("{title}", upcoming.title),
        );
  ...
  const handleCompleteReminder = async (reminder: Reminder) => {
    ...
    toast.success(`Completed: "${reminder.title}"`, {
      description: `+${reminder.points || 0} points! Great work!`,
    });
    try {
      const token = await getToken({ template: "supabase-integration" });
      await checkAchievementProgress(
        userId,
        { type: "reminder_completed", payload: { ...reminder } },
  ...
  const handleAddReminder = async () => {
    ...
    if (isToday) {
      ...
      if (dueDateTime <= now) {
        toast.error(
          "Due time cannot be in the past. Please select a future time for today.",
        );
        return;
      }

      // Also validate reminder time if it's set for today
      const [reminderHours, reminderMinutes] =
        reminderForm.reminder_time.split(":");
      const reminderDateTime = new Date(selectedDate);
      reminderDateTime.setHours(
        Number.parseInt(reminderHours),
        Number.parseInt(reminderMinutes),
        0,
        0,
      );
      ...
      if (reminderDateTime <= now) {
        toast.error(
          "Reminder time cannot be in the past. Please select a future reminder time for today.",
        );
        return;
      }
    }
    ...
    try {
      if (reminderForm.useAI) {
        ...
        const reminderData: ReminderCreateInput = {
          ...
          priority:
            reminderForm.priority === "urgent" ? "high" : reminderForm.priority,
        };
        ...
        try {
          const { ReminderEngine } = await import(
            "@/lib/reminders/ReminderEngine"
          );
          ...
          const aiReminderSuccess = await reminderEngine.scheduleSmartReminder(
            taskForAI,
            undefined,
            userId,
          );
          ...
          if (aiReminderSuccess) {
            const timeMessage = isToday
              ? "today"
              : `on ${dueDateTime.toLocaleDateString()}`;
            ...
            toast.warning("Reminder created!", {
              description:
                "Basic reminder saved, but smart timing could not be optimized.",
            });
          ...
          toast.warning("Reminder created!", {
            description:
              "Basic reminder saved, but smart timing could not be applied.",
          });
          showStuMessage("Nice! I'll remind you when it's time!", "talking");
        }
      } else {
        ...
        const timeMessage = isToday
          ? "today"
          : `for ${dueDateTime.toLocaleDateString()}`;
  ...
  const filteredReminders = reminders.filter((reminder) => {
    ...
    const completedRecently =
      isCompleted &&
      isWithinInterval(new Date(reminder.updated_at || reminder.created_at), {
        start: sevenDaysAgo,
        end: now,
      });
    ...
    switch (reminderFilter) {
      case "active":
        ...
        return (
          !isCompleted || (showCompletedFromLast7Days && completedRecently)
        );
      case "completed":
        return isCompleted;
      case "all":
    ...
    <div className="h-full overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex flex-col sm:flex-row gap-3 p-3 bg-muted/20 rounded-lg border">
              ...
              {reminderFilter === "active" && (
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setShowCompletedFromLast7Days(!showCompletedFromLast7Days)
                    }
                    className="h-8 text-xs"
                  >
                    {showCompletedFromLast7Days ? (
                      <Eye className="mr-1" />
                    ) : (
                      <EyeOff className="mr-1" />
                    )}
                    {showCompletedFromLast7Days ? "Hide" : "Show"} recent
                    completed
                  </Button>
                </div>
              )}
              ...
              <div className="text-xs text-muted-foreground flex items-center gap-1 ml-auto">
                ...
                {reminderFilter === "active" && (
                  <span>
                    | Active:{" "}
                    {organizedReminders.overdue.length +
                      organizedReminders.today.length +
                      organizedReminders.upcoming.length}
                  </span>
                )}
              </div>
            </div>
            ...
            {filteredReminders.length === 0 ? (
              <div className="text-center py-16">
                <p className="text-muted-foreground">
                  {reminderFilter === "active"
                    ? "You're all clear! Add a new reminder."
                    : reminderFilter === "completed"
                      ? "No completed reminders to show."
      ...
      >
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            ...
            {userAchievements && userAchievements.length > 0 ? (
                  ...
                  {userAchievements.map((userAchievement, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          +{userAchievement.achievements?.points_reward || 0}{" "}
                          points
                        </div>
                      </div>
                    </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">
                  Complete reminders and maintain streaks to unlock your first
                  achievements!
                </p>
              </div>
            )}
          ...
          <DialogFooter>
            <Button onClick={() => setShowAchievementsDialog(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-4 sm:space-y-6">
              ...
              <Input
                ...
                value={
                  reminderForm.due_date
                    ? format(reminderForm.due_date, "yyyy-MM-dd")
                    : ""
                }
                onChange={(e) => {
            ...
            {reminderForm.useAI && (
              <div className="text-xs sm:text-sm text-muted-foreground bg-blue-50 border border-blue-200 rounded-lg p-3">
                ...
                We space follow-ups using your patterns, priority, and
                preferences so nudges land when they help most.
              </div>
            )}
          </div>
      ...
      {/* Coin Shop Modal */}
      {showCoinShop && (
        <RewardShop variant="modal" onClose={() => setShowCoinShop(false)} />
      )}
    </div>
  );
};
...
const ReminderCard = ({
  reminder,
  onComplete,
}: { reminder: Reminder; onComplete: (r: Reminder) => void }) => {
  ...
  return (
    ...
    >
      <Card
        className={cn(
          "mb-3 transition-all",
          reminder.completed && "bg-muted/50 opacity-50",
        )}
      >
